...
<template>
  <div class="breadcrump">
    ...
    <NuxtLink :to="`/${name}`" class="cursor-pointer whitespace-nowrap pl-6' text-black dark:text-surface-0" :class="index > 0 ? 'pl-4' : ''">
      <i v-if="name == 'dashboard'" class="pi pi-objects-column"></i>
      <span v-else>{{ $t(`breadcrump.${name}`) }}</span>
    </NuxtLink>
